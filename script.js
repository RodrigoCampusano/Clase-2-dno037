...
var elSaludo;
if (0 < laHora && laHora < 8) {
    elSaludo = "Hora de dormir !! (●´ω｀●)";
} else if (7 < laHora && laHora < 23) {
    elSaludo = "Todavía no es la hora";
} else {
    elSaludo = "Queda 1 hora";
}
var elColor;
function setup() {
...
function portada() {
    createSpan(". index.html!!").parent("title");
    select("a:nth-child(1)").style("font-weight", "bold").style("color", "#ff0000");
}
function pagina() {
    createSpan(". page.html!!").parent("title");
    select("a:nth-child(2)").style("font-weight", "bold").style("color", "#ff0000");
}
